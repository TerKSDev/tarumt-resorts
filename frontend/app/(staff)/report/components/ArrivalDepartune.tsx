...
import { ChartNoAxesCombined, CheckCircle } from "lucide-react";
        ...
        {todayArrival.length === 0 ? (
          ...
          <tbody>
            {todayArrival.map((booking, index) => {
              const today = new Date().toISOString().split("T")[0];
              const isArrival = booking.checkInDate?.startsWith(today);
              ...
              return (
                ...
                >
                  <td className="py-3 px-6 text-sm font-medium">
                    <span
                      className={
                        isArrival ? "text-brand-600" : "text-amber-600"
                      }
                    >
                      {type}
                    </span>
